...
/** Pilot connection: paired clients can read health only. It never proxies local ports. */
export class RelayConnection {
  private host?: ReturnType<typeof startHost>
  private current: RelayStatus = {
    configured: false, url: 'wss://connect.routibot.com', enabled: false, state: 'disconnected', error: null,
  }
  ...
  constructor(private readonly store: Pick<Store, 'getSettings' | 'setSettings'>, private readonly hostFile: string) {
    this.http.on('upgrade', (_req, socket) => socket.destroy())
  ...
  status(): RelayStatus {
    return { ...this.current, configured: existsSync(this.hostFile) }
  }
  ...
  private start(): void {
    try {
      this.host = startHost(this.current.url, this.readDevice(), {
        onConnection: stream => this.http.emit('connection', stream),
        onState: state => {
