...
- **Connectors.** Sign in to Gmail once and every bot you allow can search, read, draft
  ...
  OAuth client.
- **Secure browser login (deferred).** A bot requests a login card in the conversation;
  you enter a username and masked password, optionally through supported password-manager
  autofill. Routi fills the intended browser form without putting credentials in the
  transcript or model context. Later, optionally remember an account for a specific bot
  and website so scheduled tasks can sign in. MFA and verification can still require
  a handover. See the [implementation proposal](docs/SECURE_BROWSER_LOGIN.md).
