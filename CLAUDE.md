...
- `daemon/` — `krogd`, Node + TypeScript. Owns credentials, the database, the bots, the
  containers. One WebSocket API on `127.0.0.1:7171`, data in `~/.krog`.
- `protocol/` — zod schemas for the wire format. `apple/Krog/Models/` is the
...
Verify with `pnpm --filter krogd typecheck`, `pnpm --filter krogd probe`, and
...
| `daemon/src/server/mcp-http.ts` | The desktop verbs as an MCP server over HTTP, at `/mcp/:botId/:conversationId`. This is how a harness we do not control gets Krog's tools. |
| `apple/Krog/Views/Settings/Providers.swift` | The roster: id, display name, brand mark, tint, whether it is wired up. `ProviderConnectPane` is the shared "account or API key" pane; Anthropic has its own because it is also the onboarding credential. |

### The three shapes an adapter comes in

1. **Vendor SDK, Krog runs the loop** — `anthropic-api.ts`, `openai-api.ts`.
   Krog owns the tool loop, so bots get the desktop directly.
...
3. **A vendor CLI runs the turn** — `anthropic-subscription.ts` (Claude Code SDK),
   ...
   exists. Krog never sees the token: the CLI holds it, and `daemon/src/auth/*-cli.ts`
...
symlinked into the isolated home so the login still counts and Krog never copies a
credential.
...
answer in the same place: approve Krog's own tools (the user granted that by giving the
...
- MCP over HTTP is accepted in `session/new` (`mcpCapabilities.http`), which is how the
  ...
  the call that arrives is `use_tool` with `rawInput.tool_name = "krog__screenshot"`.
...
5. A spike script (`daemon/scripts/spike-*.ts`, wired into `daemon/package.json`) that
   drives one real turn with a fake screen. `pnpm --filter krogd spike:grok` is the
   model for this.
